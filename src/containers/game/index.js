...
import { Button, ButtonGroup, Label, Row, Col, Grid, Well } from 'react-bootstrap'
...
import {DialogRequestReceiver, DialogRequestSender} from '../../components/modal'
...
class Game extends React.Component {
  ...
  onSquareClick = ({ ...targetSquareInfo }) => {
    ...
    const actionFlags = Utils.getActionFlags(targetSquareInfo.elements, myColor, mySelections)
  ...
  move = (col, row, piece) => {
    const { mySelections: { source }, socketId, roomId } = this.props
  ...
  render() {
    ...
    return (
      <div className={styles.gameContainer}>
        <DialogRequestReceiver className={styles.dialog} show={this.props.rematch.received} response={this.rematchResponse}/>
        <DialogRequestSender className={styles.dialog} show={this.props.rematch.sent} onRematchCancel={this.onRematchCancel}/>        
        <div className={styles.topContainer}>
          <div className={styles.boardContainer}>
            <div className={styles.boardSquareBox}>
              <Board
...
function ChessNav({ toggleOrientation, moves, myColor, isPeerConnected, iterate, history, rematch, timer}) {
  return (
    <div className={styles.controlBar}>
      <ButtonsBox onClick={toggleOrientation} iterate={iterate} rematch={rematch} moves={moves} history={history} />
      <GameTags moves={moves} myColor={myColor} isPeerConnected={isPeerConnected} history={history} timer={timer}/>
    </div>
  )
}
  ...
  return (
    <div className={styles.controlBarInfoTags}>
      ...
      <div className={styles.controlTagContainer}>
        ...
        <h1 className={styles.controlTagValue}>{history.isIterating ? history.moves : moves}</h1>
      </div>
      <div className={styles.controlTagContainer}>
        <span className={styles.controlTagHeader}>Time</span>
        <div className={styles.controlTagValue}><Timer initTime={timer}/></div>
      </div>
...
function DeadPool({ whites, blacks }) {
  return (
    <Well className={styles.killedPiecesWell}>
      <Row className={styles.killedPiecesLine}>
        <Col>
          {
            whites.map(piece => {
              return <Piece elements={{ role: piece.role, color: piece.color }} className={styles.killedPiece} />
            })
          }
        </Col>
      </Row>
